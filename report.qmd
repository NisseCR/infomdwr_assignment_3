...
- Look at head and tail of data (print as well?)
...
No NA's. We also checked for disguised missing, but found none <-- ***rewrite this bit***.
...
Why Likert scales can remain numerical:  source: [here](https://www.statisticssolutions.com/can-an-ordinal-likert-scale-be-a-continuous-variable/).

---

In addition to casting the data to the correct datatypes, the continuous predictors are transformed using a z-score standardisation. This transformation is required for one of the models (KNN), since each feature should be weighted equally when looking for neighbouring datapoints. To keep the models comparable to one another, predictor data is standardised for all the models by scaling the data prior to training process. That said, standardisation is applied separately to the `data_train` and `data_test` sets to prevent "data leakage", which would cause target information to be available during the training process whilst being unavailable when actually making predictions.
...
For this assignment, three models will be compared: a linear regression model with automatic feature selection, a KNN model with custom k value, and a random forest model. These models will be compared by their performance metric mean squared error. To ensure fair comparison and gain the mean squared error for all models, they will all be cross-validated, even if it makes less sense for the model to do so. This is particularly the case for the random forest model, as this model already tries to minimize overfitting in the building process. However, applying cross-validation to this model will yield validation results in a similar format as the other models, thus allowing for easier comparison. 

The linear regression model will be build using backward stepwise feature selection. This will be done to reduce the model's dimensionality, and thus reduce the variance of the model. The stepwise function aims to reduce the AIC error of the model, which is an estimator of prediction error. The KNN model will be built using the tunelength parameter. By supplying a tunelength parameter, multiple k values will be used to compute KNN models. Of these multiple models, the model with the lowest mean squared error will be chosen as the final KNN model. Lastly the random forest model will be cross validated, as mentioned before. The final random forest model will be the one where the number of predictors chosen for each tree is optimized, meaning the model with the number of parameters in each tree model where the mean squared error will be the lowest. 

# Model creation
We first set up the folds we want to use for the k-fold cross validation. We decided to set k to 10, thus, performing the training and validation of models 10 times over different combinations of data. Each time the model was validated on around 30 cases and train on the remaining data. We found that this data ratio as sufficient. To ensure comparability of performance of the models, the seed was set to value 123 before each training algorithm. The seed guarantees that each algorithm is performed on the same data partitions. source: [here](https://machinelearningmastery.com/compare-models-and-select-the-best-using-the-caret-r-package/).
...
Finally, the data to calculate the expected mean square error is provided by using this model to make predictions on the test set `data_test`. These predictions are
