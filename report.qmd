...
The dataset consist of records from two Portuguese high schools and includes both student marks on a mathematics course and a questionnaire concerned with the background of students, their personal life, and their study habits. The original dataset studied by Paulo Cortez and Alice Silva (2008) also included the performance of a Portuguese course with an additional 649 observations. They did research on the influence of the demographical, social, and school-related factors on students' performance. However, this will not be included in this prediction. 

They used linear regression, neural networks, Decision Trees, Support Vector Machines, and Random Forest in their analysis (Cortez and Silva, 2008, p.12). We will replicate their findings with linear regression, k-nearest neighboures, and Random Forest. To this end, we intend to find the best performing prediction for mathematical scores model on our test set. Other literature suggests that the background and social factors influence school performance of teenagers. Therefore, it is valid to perform a prediction model on this dataset, with an hypothesis that non-linear function models will perform well (idem, p.12).    

Furthermore, due to the high dimensionality (31 features) relative to the sample size (316 observations) results in two key challenges. First, the curse of dimensionality makes distance-based methods like k-Nearest Neighbors (KNN) less effective, as distances become less meaningful in high-dimensional spaces. Second, the large number of variables increases the risk of overfitting, where models capture noise rather than meaningful patterns due to insufficient data relative to the number of features. The model can become overly complex, capturing noise in the training data rather than true patterns, which leads to poor performance on unseen data. Feature selection is crucial to mitigate these issues and improve model performance.
...
Looking at the histograms and bar plots, none of the variables stand out as particularly unusual. The numerical variables show some skewness, which might be expected given their descriptions. For some numerical variables it's doubtful wheter they actually represent numerical values, as they seem more like ordered variables. Perhaps it would be best to cast `Medu` and `Fedu` to categorical variables for this reason. For the categorical variables, there are a few that seem somewhat imbalanced, but nothing that suggests any clear issues or data quality concerns. Overall, there don’t appear to be any obvious errors in the distributions, and the plots show no signs of intentionally hidden missing data either.
...
The dataset contains no missing data. Moreover, disguised missing values—such as `0`, `-999` or `"None"`—seems absent as well when investigating the data using the provided distributions for the predictor data. The only exception if `Medu` and `Fedu`, where there is a small number of `None` entries, that could represent no education or missing values. However, the proper representation is assumed. 

# Data transformation and pre-processing

In the EDA it has been discovered that some of the variables are not necessarily correctly labeled as numerical. Particularly, `Medu`, `Fedu` and  a variety of "numerical" variables using 5 point Likert scales are ordinal variables. That is because the the distance between each of their categories is not meaningful. However, studies have shown that ordinal variables with 5 or more categories, particularly in case of Likert scales, "ordinal approximation of a continuous variable"  does not negatively impact the analysis [Statistics Solutions, @stats_sol]. Effectively, the variables can be treated as continuous in the modelling process. A decision is made to leave the Likert scale variables as numerical. However, `Fedu` and `Medu` are cast to ordered factors and are not being approximated as continuous. Because the 5th category: `None` education being very small and possibly including disguised missing data, the minimal number of categories may be questioned. Additionally, the distances between different levels of education are likely less meaningful than in case of the Likert scales. 
...
## KNN model
The tuning the number of nearest neighbors - k allows for the KNN model to optimize the bias and variance tradeoff. To do so, the KNN model will be built using the `tunelength` parameter. By supplying a value to this parameter, multiple k-values will be used to compute KNN models. Of these KNN models, the model with the lowest average MSE over all the cross validation folds will be chosen as the final model. 
...
## Comparison 
The comparison of the models will be based on their performance in the K-fold cross validation. K-fold was chosen as opposed to a train and validation data split, as it reduces the chance that one of the models performed particularly well because a the validation data was particularly well suited for it, but performance on other splits would be much worse. In other words, it is a better guarantee of not overfitting the models and identifying ones who's performance is consistently good. To facilitate this comparison, the Mean Squared Error (MSE) was chosen as the the main performance metric. For MSE scores to be a fair comparison metric, the models have to be trained and tested on folds consisting of the same data points. That is the reson the random forest model will also be included in the K-fold procedure. To guarantee the algorithm is performed on the same data partitions between each model, the seed can be set right before each of the cross-validation processes [@Brownlee_2019].


# Model creation

This section contains the code used to train and k-fold cross-validate the chosen models. We first set up the folds we want to use for the k-fold cross validation. We decided to set `k` to 10, thus performing the training and validation of models 10 times over different combinations of data. Each time the model was validated on approximately 30 cases and train on the remaining data. We found that this data ratio as sufficient. To ensure comparability of performance of the models, the seed was set to value `123` before each training algorithm. 
...
The k-NN model has little hyperparameters. Besides the implimentation of cross-validation, the k-NN model will select the score of its closest neighest neighboures based on the variables in the dataset. This method will be applied for k=5 to k=23. We wanted to select 10 different k's, and thus set the tune length to 10.
...
In this study, we compared the performance of three different supervised learning models: Linear Regression with stepwise feature selection, K-Nearest Neighbours (KNN), and Random Forest. To ensure fair and robust comparisons, we used 10-fold cross-validation across all models. This technique helps avoid overfitting by training the models on different data splits and validating them on unseen partitions. 

From the boxplot visualization, we can see that Random Forest (RF) consistently achieved the lowest mean absolute error (MAE), with a median value of approximately 0.62. KNN also performed relatively well, with a median MAE around 0.66, but showed slightly more variance. Linear Regression (LM) had the highest median MAE at 0.75, indicating that it made larger errors in its predictions compared to the other two models. The RMSE results align closely with the MAE results. Random Forest (RF) achieved the lowest median RMSE of approximately 0.83, followed by KNN at 0.85, and Linear Regression (LM) with the highest RMSE at 0.91. This confirms that Random Forest made the smallest errors overall, while Linear Regression struggled with larger prediction errors. R-squared measures the proportion of variance in the dependent variable that is predictable from the independent variables. Here again, Random Forest (RF) outperformed the other models, with a median R-squared of 0.23, indicating that it explains the highest proportion of variance in the target variable. KNN had a median R-squared of 0.16, while Linear Regression (LM) had the lowest at 0.07, suggesting that it explains the least amount of variance in the data.

The cross-validation results clearly show that the Random Forest model achieved the lowest average MAE, making it the best-performing model. The KNN model came in second, while Linear Regression had the highest MAE, indicating it performed worse compared to the other models. These results are visually summarized in the boxplots and dot plots shown below, where you can observe the MAE distribution across the folds for each model.

The superior performance of the Random Forest model can be attributed to its ability to capture complex, non-linear relationships between features while controlling for overfitting through ensemble learning. Meanwhile, the KNN model also performed reasonably well, benefiting from the z-score standardization applied to ensure that all features contributed equally to distance calculations. In contrast, the Linear Regression model's simplicity might have limited its ability to handle the complex feature interactions present in the data.
...
- Nisse Hermsen: contribution to collaborative discussion on the modeling and comparison strategy 
- Isabelle de Wolf: contribution to collaborative discussion on the modeling and comparison strategy 
- Agnieszka Kubica: contribution to collaborative discussion on the modeling and comparison strategy, analysis of variable types, creation of the variable table,description of bar plots in EDA. creation and description of the correlation plot, prepossessing decisions, part of the model creation description, description of the comparison strategy, justification of casting data types in the pre-processing 
- Sara Sakhi: contribution to collaborative discussion on the modeling and comparison strategy 
- Celis Tittse: contribution to collaborative discussion on the modeling and comparison strategy 
