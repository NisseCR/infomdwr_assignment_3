...
Looking at the histograms and bar plots, none of the variables stand out as particularly unusual. The numerical variables show some skewness, which might be expected given their descriptions. For some numerical variables it's doubtful whether they actually represent numerical values, as they seem more like ordered variables. Perhaps it would be best to cast `Medu` and `Fedu` to categorical variables for this reason. For the categorical variables, there are a few that seem somewhat imbalanced, but nothing that suggests any clear issues or data quality concerns. Overall, there don’t appear to be any obvious errors in the distributions, and the plots show no signs of intentionally hidden missing data either.
...
models <- data.frame(
  model       = c("LM", "KNN", "RF"),
  RMSE        = c(0.93, 0.92, 0.86),
  MAE         = c(0.75, 0.71, 0.67),
  Rsquared    = c(0.16, 0.17, 0.26)
)
...
From the boxplot visualisation and summary table, it was observed that Random Forest (RF) consistently achieved the lowest mean absolute error (MAE), with an average value of approximately 0.67. KNN also performed relatively well, with an average MAE of 0.71, but showed slightly more variance. Linear Regression (LM) had the highest average MAE at 0.75, indicating that it made larger errors in its predictions compared to the other two models. The RMSE results align closely with the MAE results. Random Forest (RF) achieved the lowest average RMSE of approximately 0.86, followed by KNN at 0.92, and Linear Regression (LM) with the highest RMSE at 0.93. This confirms that Random Forest made the smallest errors overall, while Linear Regression struggled with larger prediction errors. R-squared measures the proportion of variance in the dependent variable that is predictable from the independent variables. Here again, Random Forest (RF) outperformed the other models, with a average R-squared of 0.26, indicating that it explains the highest proportion of variance in the target variable. KNN had an average R-squared of 0.17, while Linear Regression (LM) had the lowest at 0.16, suggesting that it explains the least amount of variance in the data.
