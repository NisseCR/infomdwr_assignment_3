...
The dataset contains quite some variables and fewer rows, which could result in a very high dimensionality . As the number of predictors grows (particularly with dummy coding), distance metrics models like KNN models become less reliable, as data points tend to spread out, making it harder to identify meaningful neighbors. Additionally, with more variables, the risk of overfitting increases. The model can become overly complex, capturing noise in the training data rather than true patterns, which leads to poor performance on unseen data.
